...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { JdProjectInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw JdProjectInvalidDataException("`$name` is not set")
            is JsonNull -> throw JdProjectInvalidDataException("`$name` is null")
            else -> throw JdProjectInvalidDataException("`$name` is invalid, received $this")
        }
